...
//    const dreal Q =
//    no2*((C_200-n1o3*k_000)*(C_200-n1o3*k_000)+(C_020-n1o3*k_000)*(C_020-n1o3*k_000)+(C_002-n1o3*k_000)*(C_002-n1o3*k_000)+no2*(C_110*C_110+C_101*C_101+C_011*C_011));
...
//jo jeste je mozny ze omega ktery kontroluje bulk viscosity musi bejt stejny jak omega pro shear viscosity aby sedela aproximace strain rate tenzoru,
//ale to musim jeste prozkoumat, zatim by to mohlo taghle slapat...
...
#define ks_210 Cs_210
		...
		const dreal Q = no2
					  * ((C_200 - n1o3 * k_000) * (C_200 - n1o3 * k_000) + (C_020 - n1o3 * k_000) * (C_020 - n1o3 * k_000)
						 + (C_002 - n1o3 * k_000) * (C_002 - n1o3 * k_000) + no2 * (C_110 * C_110 + C_101 * C_101 + C_011 * C_011));
		const dreal tau = no3 * KS.lbmViscosity + n1o2;
		const dreal omega1 = no2 / (sqrt(tau * tau + no2 * C * C * no3 * no3 * sqrt(Q) / k_000) + tau);	 //Smagorinsky SGS LES
		...
		//const dreal omega2 = no1;///(no3*KS.lbmViscosity*no2 + n1o2); // bulkViscosity > Viscosity ... test: bulkViscosity = 2/3
		// shearViscosity
