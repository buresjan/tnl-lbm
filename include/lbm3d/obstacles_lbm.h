...
void lbmDrawCube(
	LBM& lbm,
	typename LBM::map_t wall_tag,
	typename LBM::point_t phys_center,
	typename LBM::real phys_radius
)
{
	...
	for (typename LBM::idx px = lbm_center.x()-range; px <= lbm_center.x()+range; px++)
		if (px-lbm_center.x() < lbm_radius &&
			py-lbm_center.y() < lbm_radius &&
			pz-lbm_center.z() < lbm_radius )
		{
...
template< typename LBM >
void lbmDrawBoundingBox(
	LBM& lbm,
	typename LBM::map_t wall_tag,
	typename LBM::point_t phys_point1,
	typename LBM::point_t phys_point2
)
{
...
void lbmDrawCUBI(
	...
	typename LBM::map_t wall_tag,
	typename LBM::point_t phys_center,
	typename LBM::real phys_edge_length
)
{
	// bottom two cubes
	typename LBM::point_t phys_p1{
		phys_center.x() - phys_edge_length,
		phys_center.y() - phys_edge_length/2,
		phys_center.z() - phys_edge_length
	};
	typename LBM::point_t phys_p2{
		phys_center.x() + phys_edge_length,
		phys_center.y() + phys_edge_length/2,
		phys_center.z()
	};
	...
	// upper one cube
	typename LBM::point_t phys_p3{
		phys_center.x(),
		phys_center.y() - phys_edge_length/2,
		phys_center.z()
	};
	typename LBM::point_t phys_p4{
		phys_center.x() + phys_edge_length,
		phys_center.y() + phys_edge_length/2,
		phys_center.z() + phys_edge_length
	};
