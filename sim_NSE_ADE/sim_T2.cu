...
//		}
	...
	bool outputData(const BLOCK_ADE& block, int index, int dof, char *desc, idx x, idx y, idx z, real &value, int &dofs) override
	{
		...
		if (index==k++) return vtk_helper("lbm_diffusion", block.hdiffusionCoeff(x,y,z), 1, desc, value, dofs);
		if (index==k++) return vtk_helper("phys_diffusion", ade.lat.lbm2physViscosity(block.hdiffusionCoeff(x,y,z)), 1, desc, value, dofs);
		return false;
	}
};
