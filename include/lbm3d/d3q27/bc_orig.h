...
struct D3Q27_BC_All
{
	...
	{
		return mapgi != GEO_WALL && mapgi != GEO_OUTFLOW_RIGHT && mapgi != GEO_OUTFLOW_RIGHT_INTERP && mapgi != GEO_OUTFLOW_LEFT
			&& mapgi != GEO_INFLOW_TOP;
	}

	CUDA_HOSTDEV static bool isComputeDensityAndVelocity(map_t mapgi)
	{
		return mapgi != GEO_INFLOW && mapgi != GEO_INFLOW_FREE_RHO && mapgi != GEO_WALL && mapgi != GEO_SYM_TOP && mapgi != GEO_SYM_FRONT
			&& mapgi != GEO_SYM_BOTTOM && mapgi != GEO_SYM_BACK && mapgi != GEO_OUTFLOW_RIGHT && mapgi != GEO_OUTFLOW_RIGHT_INTERP
			&& mapgi != GEO_OUTFLOW_LEFT && mapgi != GEO_INFLOW_TOP;
	}
