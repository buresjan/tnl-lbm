...
int PPolynomial::parse_input_file(const char* inputfile)
{
	...
	if (! fin.good()) {
		printf("PPolynomial::parse_input_file::file %s does not exist.\n", inputfile);
		return -1;
	}
	...
	while (! fin.eof()) {
		...
		if (lines == 0) {
			...
			if (n != 1) {
				printf("PPolynomial::parse_input_file::line %d has incorrect number of tokens %d.\n", lines, n);
				return -1;
			}
...
// indexation
// -infty .. m_divisions[0] .. 0 .. m_divisions[1] .. 1 .. ... .. m_pieces-2 .. m_divisions[m_pieces-1] .. m_pieces-1 .. m_divisions[m_pieces] ..
// m_pieces-1 .. +infty
...
PPolynomial::PPolynomial(const char* inputfilename)
{
	...
	if (parse_input_file(inputfilename) < 0)
		m_FLAG_allocated = false;
}
